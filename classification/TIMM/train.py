...
def save_batch_images(input_tensor, save_dir='saved_images'):
    """
    Save a batch of images from tensor format
    input_tensor: torch.Size([32, 3, 224, 224]) - can be on GPU or CPU
    """   
    ...
    # Create normalization tensors on the SAME device as input
    ...
    for i in range(batch_size):
        # Get single image
        img = input_tensor[i]  # Shape: [3, 224, 224]
        
        # Denormalize (both tensors now on same device)
        img_denorm = img * std + mean
        img_denorm = torch.clamp(img_denorm, 0, 1)
        
        # Move to CPU for PIL conversion
        img_cpu = img_denorm.cpu()
