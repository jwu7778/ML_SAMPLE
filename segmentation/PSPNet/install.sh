...
conda run -n "$env_name" pip install -U importlib_metadata huggingface_hub future tensorboard ftfy onnxruntime-gpu
...
tar xf stanford_background.tar.gz -C mmsegmentation
conda run -n "$env_name" python convertstanford.py
