...
@TRANSFORMS.register_module()
class ConvertMaskForAlbu(BaseTransform):
    def __init__(self, num_classes):
        self.num_classes = num_classes
    
    def transform(self, results):
        # Convert single-channel mask to a list of binary masks (one per class)
        gt_seg = results['gt_seg_map']
        h, w = gt_seg.shape
    
        # Create multi-channel mask
        multi_channel_mask = np.zeros((h, w, self.num_classes), dtype=np.uint8)
        for class_id in range(self.num_classes):
            multi_channel_mask[:, :, class_id] = (gt_seg == class_id).astype(np.uint8)
        
        results['gt_seg_map'] = multi_channel_mask
        return results

@TRANSFORMS.register_module()
class ConvertMaskFromAlbu(BaseTransform):
    def transform(self, results):
        # Convert list of binary masks (H, W) back to single-channel mask (H, W) with class indices
        if 'gt_seg_map' in results:
            masks = results['gt_seg_map']
            # Convert back to single channel
            gt_seg = np.argmax(masks, axis=2).astype(np.uint8)
